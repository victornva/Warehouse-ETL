from data_transfer8 import DataTransfer
import csv
import psycopg2
...
#        self, source_pg_conn_str, source_pg_meta_conn_str, query, *args, **kwargs
    ):
        super(DataTransferPostgres, self).__init__(
            source_pg_conn_str=source_pg_conn_str,query=query, *args, **kwargs
#            source_pg_conn_str=source_pg_conn_str,source_pg_meta_conn_str=pg_meta_conn_str,query=query, *args, **kwargs
        )
        self.source_pg_conn_str = source_pg_conn_str
#        self.source_pg_meta_conn_str = source_pg_meta_conn_str
        self.query = query
    ...
    def provide_data(self, csv_file, context):
        ...
        while True:
            rows = pg_cursor.fetchmany(size=10)
            if rows:
                for row in rows:
                    _row = list(row)
