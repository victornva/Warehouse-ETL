...
Поэтому я так подумал, что в качестве ДЗ лучше подойдёт кусок реального модуля (он и сейчас работает), хотя проверить его работу к сожалению не получится 
т.к. работает он в закрытой сети.
Ниже привожу кусок кода который подключается к базе сервера MySQL (источник), считывает свежие данные (CDR) с учётом набора критериев, 
затем подключается к базе PostgresSQL (хранилище) и записывает туда полученные данные. Считанные в источнике данные в конце метятся как прочитанные (так надо по условиям задачки).
